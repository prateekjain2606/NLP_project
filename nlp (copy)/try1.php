...
if(isset($_POST["dell1"]))
{
	...
	$de1 = $_POST["de1"];
	$selection_of_word = "SELECT * FROM wordssentences2 WHERE english='$de1'";
	...
	{
		$selection_of_word = "DELETE FROM wordssentences2 WHERE english='$de1'";
...
if(isset($_POST["dell2"]))
{
	...
	if (!$conn)
		echo "Connection failed<br>";
	$de2 = $_POST["de2"];
	$selection_of_word = "SELECT * FROM wordssentences2 WHERE hindi='$de2'";
	$result=mysqli_query($conn,$selection_of_word);
	if($result->num_rows == 0)
	{
		echo '<form action = "try1.php" method = "post">
	...
	{
		$selection_of_word = "DELETE FROM wordssentences2 WHERE hindi='$de2'";
		mysqli_query($conn,$selection_of_word);
		echo '<form action = "try1.php" method = "post">
...
if(isset($_POST["cont"]))
{
	...
	$id1 = $_POST["id1"];
	$id2 = $_POST["id2"];
	$selection_of_word = "SELECT * FROM wordssentences2 WHERE english='$id1' AND hindi='$id2'";
	$result=mysqli_query($conn,$selection_of_word);
	if($result->num_rows > 0)
	{
		echo '<form action = "try1.php" method = "post">
	...
	{
		$selection_of_word = "INSERT INTO wordssentences2(english,hindi) VALUES('$id1','$id2')";
		mysqli_query($conn,$selection_of_word);
		echo '<form action = "try1.php" method = "post">
...
if(isset($_POST["isposted"]))
{
	...
	$y = $_POST["y"];
	$x = $_POST["x"];
	$selection_of_word = "SELECT * FROM wordssentences2 WHERE hindi='$y'";
	$result=mysqli_query($conn,$selection_of_word);
	if($result->num_rows == 0)
	{
		echo '<center><span>Sorry no such word exists in database:<br><br></span></center>';
	}
	echo "<table bgcolor='white' border='2' opacity=0.8>";
	
	while($row=mysqli_fetch_row($result))
	{	echo "<tr><td>".$row[0]."</td><td> ".$y."</td><td> ".$x."</td></tr>";	if($row[2]!=""){
			echo "<br><span>".$row[2].".</span><br><br>";
			echo "<span>".$row[3]."|</span><br><br>";
		}
	}
	echo "</table>";
...
if(isset($_POST["ispost"]))
{
	...
	if (!$conn)
		echo "Connection failed<br>";
	$z = $_POST["z"];
	$selection_of_word = "SELECT hindi FROM wordssentences2 WHERE english='$z'";
	$result=mysqli_query($conn,$selection_of_word);
	if($result->num_rows == 0)
	{
		echo '<form action = "try1.php" method = "post">
